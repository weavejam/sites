// Create GitHub issues for tooldone migration batches.
//
// Usage:
...
const LABELS = ["tooldone-port", "ready"];
...
async function main() {
  ...
  for (const b of toCreate) {
    ...
    const title = `port(tooldone): ${id} — ${b.jobs.map((j) => `${j.category}/${j.toolId}`).join(", ")}`.slice(0, 250);
