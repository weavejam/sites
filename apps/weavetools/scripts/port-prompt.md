...
- Target app:    `D:\shudu\shudu\apps\weavetools\`
...
3. Append a new `ToolEntry` to `src/data/tools/{{CATEGORY}}.ts` with:
   ...
   - English title rules (SEO-optimized, like the legacy weavetools site):
     * NO brand suffix. NO "— Fast & Free Online Tool", no "WeaveTools".
...
- All input labels, button text, unit names, error messages routed
  through `t(...)`.
- Code passes `pnpm --filter @weavejam/weavetools build` (no TS errors).
