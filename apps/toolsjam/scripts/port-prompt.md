...
new app at `apps/toolsjam/`. This run is fully non-interactive — read the
inputs, write the files, exit. Do not ask questions.

# Context

- Monorepo root: `D:\shudu\shudu`
- Target app:    `D:\shudu\shudu\apps\toolsjam\`
...
3. Append a new `ToolEntry` to `src/data/tools/{{CATEGORY}}.ts` with:
   ...
   - English title rules (SEO-optimized, like the legacy toolsjam site):
     * NO brand suffix. NO "— Fast & Free Online Tool", no "ToolsJam".
...
- All input labels, button text, unit names, error messages routed
  through `t(...)`.
- Code passes `pnpm --filter @weavejam/toolsjam build` (no TS errors).
