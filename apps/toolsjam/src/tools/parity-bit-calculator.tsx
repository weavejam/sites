...
function computeParityBit(data: string, type: ParityType): 0 | 1 {
...
export default function ParityBitCalculator(_props: { locale: Locale }) {
  const t = useTranslations("tool.parity-bit-calculator");
  ...
  const result = React.useMemo(() => {
    ...
    const parityBit = computeParityBit(cleanData, parityType);
    const transmissionString = cleanData + parityBit.toString();
    return { ones, parityBit, transmissionString };
  }, [cleanData, dataValid, parityType]);

  const validationResult = React.useMemo(() => {
    if (cleanReceived === "" || !isValidBinary(cleanReceived)) return null;
    const totalOnes = countOnes(cleanReceived);
    const valid =
      parityType === "even" ? totalOnes % 2 === 0 : totalOnes % 2 === 1;
    return { valid, totalOnes };
  }, [cleanReceived, parityType]);
  ...
  const showDataError = touched && binaryData.trim() !== "" && !isValidBinary(cleanData);
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="pbc-data">{t("field.binaryData")}</Label>
              <Input
                id="pbc-data"
                type="text"
                inputMode="text"
                value={binaryData}
                placeholder={t("field.placeholder.binaryData")}
                onChange={(e) => {
            ...
            <div className="space-y-2">
              <Label htmlFor="pbc-received">{t("field.receivedData")}</Label>
              <Input
                id="pbc-received"
                type="text"
                inputMode="text"
                value={receivedData}
                placeholder={t("field.placeholder.receivedData")}
                onChange={(e) => {
