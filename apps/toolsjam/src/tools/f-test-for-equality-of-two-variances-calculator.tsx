...
export default function FTestEqualityVariancesCalculator(_props: { locale: Locale }) {
  const t = useTranslations("tool.f-test-for-equality-of-two-variances-calculator");
  ...
  const v1 = parseFloat(var1);
  const n1Num = parseInt(n1, 10);
  const v2 = parseFloat(var2);
  ...
  const result = React.useMemo(() => {
    ...
    const isFirstLarger = v1 >= v2;
    const larger = isFirstLarger ? v1 : v2;
    const smaller = isFirstLarger ? v2 : v1;
  ...
  }, [valid, v1, n1Num, v2, n2Num, alphaNum]);

  function reset() {
    setVar1(""); setN1(""); setVar2(""); setN2("");
    setAlpha("0.05"); setTouched(false);
  }

  function loadExample(v1s: string, n1s: string, v2s: string, n2s: string, a: string) {
    setVar1(v1s); setN1(n1s); setVar2(v2s); setN2(n2s);
    ...
    <div className="space-y-10">
        ...
        <CardContent className="space-y-6">
          <div className="grid gap-6 sm:grid-cols-2">
            <div className="space-y-4">
              ...
              <Label htmlFor="ftev-v1">{t("field.variance1")}</Label>
                <Input
                  id="ftev-v1" type="number" inputMode="decimal" min="0"
                  value={var1} placeholder={t("placeholder.variance")}
                  onChange={(e) => { setVar1(e.target.value); setTouched(true); }}
                />
              </div>
              <div className="space-y-2">
              <Label htmlFor="ftev-n1">{t("field.sampleSize1")}</Label>
                <Input
                  id="ftev-n1" type="number" inputMode="numeric" min="2"
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
              <Label htmlFor="ftev-v2">{t("field.variance2")}</Label>
                <Input
                  id="ftev-v2" type="number" inputMode="decimal" min="0"
                  value={var2} placeholder={t("placeholder.variance")}
                  onChange={(e) => { setVar2(e.target.value); setTouched(true); }}
                />
              </div>
              <div className="space-y-2">
              <Label htmlFor="ftev-n2">{t("field.sampleSize2")}</Label>
                <Input
                  id="ftev-n2" type="number" inputMode="numeric" min="2"
          ...
          <div className="space-y-2">
            <Label htmlFor="ftev-alpha">{t("field.alpha")}</Label>
            <select
              id="ftev-alpha" value={alpha}
      ...
      <section className="space-y-4">
        ...
        <div className="flex flex-wrap gap-2 pt-2">
          ...
          <Button type="button" variant="outline" size="sm"
            onClick={() => loadExample("110", "41", "125", "31", "0.05")}>
            {t("examples.loadTeaching")}
          </Button>
          <Button type="button" variant="outline" size="sm"
            onClick={() => loadExample("5.2", "100", "4.8", "100", "0.01")}>
