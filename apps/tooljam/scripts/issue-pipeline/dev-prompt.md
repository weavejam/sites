...
site `tooldone.com` into `apps/tooljam/`. You are NON-INTERACTIVE: do all work,
do not ask questions, exit when done.

# Context

- Worktree root: <REPO_ROOT>  (THIS worktree — ALL writes go here)
- Target app:    <REPO_ROOT>/apps/tooljam/
...
- HTML snapshots: under
  `D:\shudu\shudu\apps\tooljam\.scrape\html\<category>\<slug>.html`
...
inside `<REPO_ROOT>/apps/tooljam`. This is the ONLY shell command you run.
...
You are a code + SEO reviewer for batch {{BATCH_ID}} of the tooljam migration
sitting at `<REPO_ROOT>/apps/tooljam`. The pages added in this batch are:
