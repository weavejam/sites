...
export default function BuoyancyCalculator(_props: { locale: Locale }) {
  const t = useTranslations("tool.buoyancy-calculator");
  ...
  const results = React.useMemo(() => {
    ...
    return { buoyantForce, weight, netForce };
  }, [m, V, rho, g, allValid]);

  function statusLabel(): string {
    if (!results) return "";
    if (Math.abs(results.netForce) < 0.0001) return t("result.neutral");
    return results.netForce > 0 ? t("result.floats") : t("result.sinks");
  }
        ...
        <CardContent className="space-y-6">
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="bc-mass">{t("field.mass")}</Label>
              <Input
                id="bc-mass"
                type="number"
                inputMode="decimal"
            ...
            <div className="space-y-2">
              <Label htmlFor="bc-volume">{t("field.volume")}</Label>
              <Input
                id="bc-volume"
                type="number"
                inputMode="decimal"
            ...
            <div className="space-y-2">
              <Label htmlFor="bc-fluid">{t("field.fluidDensity")}</Label>
              <Input
                id="bc-fluid"
                type="number"
                inputMode="decimal"
            ...
            <div className="space-y-2">
              <Label htmlFor="bc-gravity">{t("field.gravity")}</Label>
              <Input
                id="bc-gravity"
                type="number"
                inputMode="decimal"
          ...
          {results !== null && (
            <div className="rounded-lg border border-zinc-200 bg-zinc-50 p-4 space-y-3">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div>
                  <div className="text-xs text-zinc-500">{t("result.weight")}</div>
                  <div className="text-xl font-semibold text-zinc-900">
                    {fmt(results.weight)} <span className="text-sm font-normal">{t("result.weightUnit")}</span>
                  </div>
                </div>
                <div>
