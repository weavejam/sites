...
## Overview
Port {{N_PAGES}} calculator pages from legacy tooldone.com into `apps/weavetools/`, **including all 10 locales**. English content is authored by hand from the scraped HTML; the other 9 locales (`zh-CN`, `zh-TW`, `ja`, `ko`, `es`, `fr`, `de`, `pt`, `ru`) are filled by the bundled `translate-tool.ts` (gpt-5.4-mini) before tests run.

## Motivation
Part of the full tooldone.com → apps/weavetools migration (~568 batches × 5 pages = 2840 pages total). Each batch ships as one reviewable, revertable PR that contains the implementation + all translations.
...
8. `gh pr create --title "port(weavetools): {{BATCH_ID}}" --body "Closes #{{ISSUE_NUMBER}}\n\n<review report>"`
