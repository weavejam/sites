...
export default function ArctanCalculator(_props: { locale: Locale }) {
  const t = useTranslations("tool.arctan-calculator");
  const [inputValue, setInputValue] = React.useState<string>("");
  const [outputUnit, setOutputUnit] = React.useState<OutputUnit>("degrees");
  const [touched, setTouched] = React.useState(false);

  const xNum = parseFloat(inputValue);
  const inputValid = inputValue !== "" && Number.isFinite(xNum);

  const result = React.useMemo<number | null>(() => {
    if (!inputValid) return null;
    const rad = Math.atan(xNum);
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          <div className="space-y-2">
            <Label htmlFor="arctan-input">{t("field.value")}</Label>
            <Input
              id="arctan-input"
              type="number"
              inputMode="decimal"
