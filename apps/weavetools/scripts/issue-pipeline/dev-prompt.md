...
- Target app:    <REPO_ROOT>/apps/weavetools/
...
- HTML snapshots: under
  `D:\shudu\shudu\apps\weavetools\.scrape\html\<category>\<slug>.html`
...
inside `<REPO_ROOT>/apps/weavetools`. This is the ONLY shell command you run.
...
Spawn ONE `task` sub-agent with these parameters:
  ...
  - model: "gpt-5.5"
  - name: "weavetools-batch-reviewer"
...
You are a code + SEO reviewer for batch {{BATCH_ID}} of the weavetools migration
sitting at `<REPO_ROOT>/apps/weavetools`. The pages added in this batch are:
