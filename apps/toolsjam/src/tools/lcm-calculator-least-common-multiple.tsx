...
export default function LcmCalculatorLeastCommonMultiple(
  _props: { locale: Locale }
) {
  const t = useTranslations("tool.lcm-calculator-least-common-multiple");
  const [input, setInput] = React.useState("");
  const [touched, setTouched] = React.useState(false);

  const nums = React.useMemo(() => parseNumbers(input), [input]);
  const isValid = nums !== null;

  const result = React.useMemo<number | null>(() => {
    if (!isValid || !nums) return null;
    return computeLCM(nums);
  }, [isValid, nums]);
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          <div className="space-y-2">
            <Label htmlFor="lcm-numbers">{t("field.numbers")}</Label>
            <Input
              id="lcm-numbers"
              type="text"
              value={input}
