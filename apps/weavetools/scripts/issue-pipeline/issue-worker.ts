// One-issue worker. Picks an open `weavetools-port` issue with `ready` label,
...
async function claimIssue(specific: number | null, workerId: string): Promise<OpenIssue | null> {
  ...
  try {
    const queryArgs = specific
      ...
      : ["issue", "list", "--label", "weavetools-port", "--label", "ready", "--state", "open", "--json", "number,title,labels", "--limit", "100"];
...
async function main() {
  ...
  try {
    ...
    const msg = `port(weavetools): ${batchId} — ${ids}\n\nCloses #${issue.number}\n\nCo-authored-by: Copilot <223556219+Copilot@users.noreply.github.com>`;
    ...
    const prCreate = run("gh", ["pr", "create", "--title", JSON.stringify(`port(weavetools): ${batchId}`), "--body-file", JSON.stringify(prBodyFile), "--base", "main", "--head", branch], wt, wid);
