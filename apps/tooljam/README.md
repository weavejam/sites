...
```pwsh
pnpm install
pnpm --filter @weavejam/tooljam dev
```
...
pnpm --filter @weavejam/tooljam scrape

# Port a single tool by URL slug (English path).
pnpm --filter @weavejam/tooljam port-page math/percentage-calculator

# Batch a list.
pnpm --filter @weavejam/tooljam port-page --batch .scrape/pilot.txt
```
...
```pwsh
pnpm --filter @weavejam/tooljam deploy
```
