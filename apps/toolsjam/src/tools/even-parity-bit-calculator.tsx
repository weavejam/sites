...
/** Returns 0 if count of 1s is already even; 1 if count is odd. */
function computeEvenParityBit(data: string): 0 | 1 {
  const ones = countOnes(data);
  return ones % 2 === 0 ? 0 : 1;
}

export default function EvenParityBitCalculator(_props: { locale: Locale }) {
  const t = useTranslations("tool.even-parity-bit-calculator");
  ...
  const receivedValid =
    cleanReceived === "" || isValidBinary(cleanReceived);

  const result = React.useMemo(() => {
    if (!dataValid) return null;
    const ones = countOnes(cleanData);
    const parityBit = computeEvenParityBit(cleanData);
  ...
  const validationResult = React.useMemo(() => {
    ...
    return { valid: totalOnes % 2 === 0, totalOnes };
  }, [cleanReceived]);
  ...
  const showDataError =
    touched && binaryData.trim() !== "" && !isValidBinary(binaryData.trim());
        ...
        <CardContent className="space-y-6">
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="epbc-data">{t("field.binaryData")}</Label>
              <Input
                id="epbc-data"
                type="text"
                inputMode="text"
            ...
            <div className="space-y-2">
              <Label htmlFor="epbc-received">{t("field.receivedData")}</Label>
              <Input
                id="epbc-received"
                type="text"
                inputMode="text"
