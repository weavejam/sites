...
```pwsh
pnpm install
pnpm --filter @weavejam/toolsjam dev
```
...
pnpm --filter @weavejam/toolsjam scrape

# Port a single tool by URL slug (English path).
pnpm --filter @weavejam/toolsjam port-page math/percentage-calculator

# Batch a list.
pnpm --filter @weavejam/toolsjam port-page --batch .scrape/pilot.txt
```
...
```pwsh
pnpm --filter @weavejam/toolsjam deploy
```
