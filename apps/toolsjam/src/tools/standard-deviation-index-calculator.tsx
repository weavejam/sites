...
function computeSD(nums: number[]): SDResult | null {
  const n = nums.length;
  if (n < 2) return null;
  ...
  const cv = mean !== 0 ? (sampleSD / Math.abs(mean)) * 100 : NaN;
  return {
    n,
    mean,
    sum,
...
export default function StandardDeviationIndexCalculator(_props: {
  locale: Locale;
}) {
  const t = useTranslations("tool.standard-deviation-index-calculator");
  ...
  const result = React.useMemo<SDResult | null>(() => {
    if (!valid) return null;
    return computeSD(nums);
  }, [valid, nums]);
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          <div className="space-y-2">
            <Label htmlFor="sdi-data">{t("field.data")}</Label>
            <textarea
              id="sdi-data"
              className="border-input placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-ring/50 dark:bg-input/30 flex min-h-[6rem] w-full rounded-md border bg-transparent px-3 py-2 text-base shadow-xs transition-[color,box-shadow] outline-none focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50 md:text-sm h-32"
              value={dataStr}
              ...
              <div className="grid gap-2 grid-cols-2 sm:grid-cols-3 text-sm">
                {(
                  [
                    ...
                    ["result.sampleSD", result.sampleSD],
                    ["result.populationSD", result.populationSD],
                    ["result.sampleVariance", result.sampleVariance],
                    ["result.populationVariance", result.populationVariance],
                    ["result.cv", result.cv],
                  ] as [string, number][]
                ).map(([key, val]) => (
                  <div
                  ...
                  >
                    ...
                    <div className="font-semibold">
                      {fmt(val)}
                      {key === "result.cv" && Number.isFinite(val) ? "%" : ""}
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            ...
            <Button
              ...
              size="sm"
              onClick={() =>
                loadExample("150.25, 152.50, 149.75, 153.00, 151.50")
              }
            >
