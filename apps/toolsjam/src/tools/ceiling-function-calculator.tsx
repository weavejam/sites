...
export default function CeilingFunctionCalculator(_props: { locale: Locale }) {
  const t = useTranslations("tool.ceiling-function-calculator");
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          <div className="space-y-2">
            <Label htmlFor="ceil-input">{t("field.inputNumber")}</Label>
            <Input
              id="ceil-input"
              type="number"
              inputMode="decimal"
          ...
          {result !== null && !showError && (
            <div className="rounded-lg border border-zinc-200 bg-zinc-50 p-4 space-y-2">
              ...
              <div className="text-3xl font-semibold text-zinc-900">
                {t("result.equation", { x: num.toString(), result: result.toString() })}
              </div>
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            <Button type="button" variant="outline" size="sm" onClick={() => loadExample("2.3")}>
              {t("examples.load1")}
            </Button>
            <Button type="button" variant="outline" size="sm" onClick={() => loadExample("-1.7")}>
              {t("examples.load2")}
            </Button>
            <Button type="button" variant="outline" size="sm" onClick={() => loadExample("5")}>
              {t("examples.load3")}
            </Button>
          </div>
        </CardContent>
