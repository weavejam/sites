// Create GitHub issues for tooljam migration batches.
//
// Usage:
...
const LABELS = ["tooljam-port", "ready"];
...
async function main() {
  ...
  for (const b of toCreate) {
    ...
    const title = `port(tooljam): ${id} — ${b.jobs.map((j) => `${j.category}/${j.toolId}`).join(", ")}`.slice(0, 250);
