...
const OPS: Op[] = ["add", "subtract", "multiply", "divide"];
...
export default function DecimalCalculator(_props: { locale: Locale }) {
  const t = useTranslations("tool.decimal-calculator");
  ...
  const result = React.useMemo<number | null>(() => {
    ...
    switch (op) {
      case "add":      return aNum + bNum;
      case "subtract": return aNum - bNum;
      case "multiply": return aNum * bNum;
      case "divide":   return bNum === 0 ? null : aNum / bNum;
    }
  }, [aValid, bValid, aNum, bNum, op]);

  const divByZero = touched && aValid && bValid && op === "divide" && bNum === 0;
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="dc-a">{t("field.firstNumber")}</Label>
              <Input
                id="dc-a"
                type="number"
                inputMode="decimal"
            ...
            <div className="space-y-2">
              <Label htmlFor="dc-b">{t("field.secondNumber")}</Label>
              <Input
                id="dc-b"
                type="number"
                inputMode="decimal"
          ...
          {result !== null && !divByZero && (
            <div className="rounded-lg border border-zinc-200 bg-zinc-50 p-4 space-y-2">
              <div className="text-sm font-medium text-zinc-500">{t("result.heading")}</div>
              <div className="text-2xl font-semibold text-zinc-900">
                {fmt(aNum)} {opSymbol(op)} {fmt(bNum)} = {fmt(result)}
              </div>
            </div>
          )}
        </CardContent>
