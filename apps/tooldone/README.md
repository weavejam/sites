...
```pwsh
pnpm install
pnpm --filter @weavejam/tooldone dev
```
...
pnpm --filter @weavejam/tooldone scrape

# Port a single tool by URL slug (English path).
pnpm --filter @weavejam/tooldone port-page math/percentage-calculator

# Batch a list.
pnpm --filter @weavejam/tooldone port-page --batch .scrape/pilot.txt
```
...
```pwsh
pnpm --filter @weavejam/tooldone deploy
```
