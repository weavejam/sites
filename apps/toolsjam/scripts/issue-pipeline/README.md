# toolsjam issue-pipeline runbook

Parallel, issue-driven migration of 2840 calculator pages from legacy
`tooldone.com` into `apps/toolsjam/` using Copilot CLI workers. One
...
- When rewriting brand names that share substrings with placeholders, beware case-insensitive uppercase rules corrupting placeholders mid-pipeline (e.g. `___TOOLDONE_DOT_COM___` → `___TOOLSJAM_DOT_COM___`).
