...
type CorrectionRate = "conservative" | "moderate" | "aggressive";
type FluidType = "ns" | "hts" | "halfNs";

const CORRECTION_RATES: Record<CorrectionRate, number> = {
  conservative: 0.5,
  moderate: 1.0,
  aggressive: 2.0,
};
...
export default function SodiumCorrectionCalculator(_props: { locale: Locale }) {
  const t = useTranslations("tool.sodium-correction-calculator");

  const [currentNa, setCurrentNa] = React.useState("");
  const [desiredNa, setDesiredNa] = React.useState("");
  const [weight, setWeight] = React.useState("");
  const [gender, setGender] = React.useState<Gender>("male");
  const [corrRate, setCorrRate] = React.useState<CorrectionRate>("conservative");
  ...
  const result = React.useMemo<CalcResult | null>(() => {
    ...
    if (desNum > curNum && fluidNa <= curNum) return null;
    const tbw = wNum * (gender === "male" ? 0.6 : 0.5);
    const deficit = tbw * (desNum - curNum);
    const absDelta = Math.abs(desNum - curNum);
    const rate = CORRECTION_RATES[corrRate];
    const correctionTime = absDelta / rate;
    if (deficit === 0) return null;
    // For hypernatremia (desNum < curNum), fluid Na must be below current Na
    if (desNum < curNum && fluidNa >= curNum) return null;
    // For hyponatremia (desNum > curNum), fluid Na must be above current Na
    if (desNum > curNum && fluidNa <= curNum) return null;
    const volume = (deficit / (fluidNa - curNum)) * 1000;
    const infusionRate = Math.abs(volume) / correctionTime;
    return { tbw, deficit, volume, correctionTime, infusionRate };
  }, [curValid, desValid, wValid, curNum, desNum, wNum, gender, corrRate, fluidType]);
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              <Label htmlFor="scc-gender">{t("field.gender")}</Label>
              <select
                id="scc-gender"
            ...
            <div className="space-y-2">
              <Label htmlFor="scc-rate">{t("field.correctionRate")}</Label>
              <select
                id="scc-rate"
                className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-xs outline-none"
                value={corrRate}
                onChange={(e) =>
                  setCorrRate(e.target.value as CorrectionRate)
                }
              >
                <option value="conservative">
                  {t("correctionRate.conservative")}
                </option>
                <option value="moderate">
                  {t("correctionRate.moderate")}
                </option>
                <option value="aggressive">
                  {t("correctionRate.aggressive")}
                </option>
              </select>
            </div>
            <div className="space-y-2">
              <Label htmlFor="scc-fluid">{t("field.fluidType")}</Label>
              <select
                id="scc-fluid"
                className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-xs outline-none"
                value={fluidType}
              ...
              >
                ...
                <option value="halfNs">{t("fluidType.halfNs")}</option>
              </select>
            </div>
          </div>
              ...
              <div className="grid gap-2 sm:grid-cols-2 text-sm">
                ...
                <div>
                  ...
                  <span className="font-semibold text-base">
                    {fmt(Math.abs(result.volume), 0)}{" "}
                    {t("result.volumeUnit")}
                  </span>
                </div>
                <div>
                  <span className="text-zinc-500">{t("result.time")}: </span>
                  <span className="font-semibold">
                    {fmt(result.correctionTime)} {t("result.timeUnit")}
                  </span>
                </div>
                <div>
                  <span className="text-zinc-500">{t("result.rate")}: </span>
                  <span className="font-semibold">
                    {fmt(result.infusionRate, 0)} {t("result.rateUnit")}
                  </span>
                </div>
              </div>
            </div>
