// Create GitHub issues for toolsjam migration batches.
//
// Usage:
...
const LABELS = ["toolsjam-port", "ready"];
...
async function main() {
  ...
  for (const b of toCreate) {
    ...
    const title = `port(toolsjam): ${id} — ${b.jobs.map((j) => `${j.category}/${j.toolId}`).join(", ")}`.slice(0, 250);
